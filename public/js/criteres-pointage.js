...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (formDepartemental) {
        ...
        formDepartemental.addEventListener('submit', async function(e) {
            e.preventDefault();
            ...
            try {
                const formData = new FormData(formDepartemental);
                const response = await fetch(formDepartemental.action, {
                    method: 'POST',
                    headers: {
                ...
                if (data.success) {
                    ...
                    const bsModal = bootstrap.Modal.getInstance(document.getElementById('departementModal'));
                    bsModal.hide();
                    formDepartemental.reset();
    ...
    if (historyModal) {
        historyModal.addEventListener('show.bs.modal', async function () {
            ...
            try {
                ...
                const response = await fetch('/criteres-pointage/historique-departement', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                    },
                    body: JSON.stringify({ departement_id: departementId })
                });
                const data = await response.json();
                if (data.success) {
                    let html = '';
                    data.criteres.forEach(c => {
                        html += `<tr>
                            ...
                            <td>${c.date_fin}</td>
                            <td>${c.periode}</td>
                            <td>${c.nombre_pointages}</td>
                            <td>${c.created_at}</td>
                            <td>${c.createur ?? ''}</td>
                            <td>${c.actif ? '<span class="badge bg-success">Actif</span>' : '<span class="badge bg-secondary">Inactif</span>'}</td>
                        </tr>`;
                    });
                    if (!html) html = '<tr><td colspan="7" class="text-center">Aucun critère trouvé.</td></tr>';
                    tbody.innerHTML = html;
                } else {
                    tbody.innerHTML = '<tr><td colspan="7" class="text-center text-danger">Erreur lors du chargement.</td></tr>';
                }
            } catch (e) {
                console.error(e);
                tbody.innerHTML = '<tr><td colspan="7" class="text-center text-danger">Erreur lors du chargement.</td></tr>';
